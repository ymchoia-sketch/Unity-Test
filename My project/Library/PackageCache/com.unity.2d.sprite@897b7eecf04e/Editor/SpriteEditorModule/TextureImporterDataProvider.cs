...
namespace UnityEditor.U2D.Sprites
{
    ...
    internal class TextureImporterDataProvider : ISpriteEditorDataProvider, ISpriteNameFileIdDataProvider
    {
        ...
        public Dictionary<string, string> customMetadata => m_CustomMetadata;
        ...
        void ISpriteEditorDataProvider.Apply()
        {
            ...
            var spriteSheet = so.FindProperty("m_SpriteSheet");
            var spriteSheetSprites = spriteSheet.FindPropertyRelative("m_Sprites");
            ...
            if (spriteSheetSprites.arraySize > 0)
            {
                var validateCurrent = spriteSheetSprites.GetArrayElementAtIndex(0);
                for (int i = 0; i < spriteSheetSprites.arraySize; ++i)
                {
                    ...
                    {
                        spriteSheetSprites.DeleteArrayElementAtIndex(i);
                        --i;
                    }
                }
            }
            ...
            if (newCount > 0)
            {
                var idx = spriteSheetSprites.arraySize;
                spriteSheetSprites.arraySize += newCount;
                var addCurrent = spriteSheetSprites.GetArrayElementAtIndex(idx);
                foreach (var newSprite in newSprites.Values)
                {
            ...
            var pairsSo = spriteSheet.FindPropertyRelative("m_NameFileIdTable");
            ...
            SpriteCustomDataTransfer.Apply(spriteSheet, m_CustomMetadata);
        ...
        void ISpriteEditorDataProvider.InitSpriteEditorDataProvider()
        {
            ...
            if (spriteSheetSprites.arraySize > 0)
            {
                var sp = spriteSheetSprites.GetArrayElementAtIndex(0);
                for (var i = 0; i < spriteSheetSprites.arraySize; ++i)
                {
            ...
            m_CustomMetadata = SpriteCustomDataTransfer.Load(spriteSheet);
